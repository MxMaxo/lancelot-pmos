...
1. **Проверить r24** (LEVEL_LOW + без cluster-sleep + батарея + зарядка + сглаживание):
   стабильно 10+ мин, IRQ 172 = 0/с, `fuel-gauge/capacity` — % заряда.
   **Fuel gauge — вольтметровый** (по VBAT + OCV-таблица), поэтому без сглаживания %
   «прыгал» (напряжение проседает под нагрузкой). Добавлен EMA в `gab_work`
   (опрос каждые 10 с, α=1/4) + кэш напряжения → % стабильный. Ограничение осталось:
   на зарядке держит 100% (float 4.38В), при отключении честно показывает ~94-96%.
